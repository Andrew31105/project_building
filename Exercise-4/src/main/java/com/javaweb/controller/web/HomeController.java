...
public class HomeController {
	...
	public ModelAndView homePage(BuildingSearchRequest buildingSearchRequest, HttpServletRequest request) {
        ...
        Integer pageParam = buildingSearchRequest.getPage();
        if (pageParam == null || pageParam < 1) {
            buildingSearchRequest.setPage(1);
        }
        buildingSearchRequest.setLimit(3);
        BuildingDTO buildingDTO = new BuildingDTO();
        if (buildingSearchRequest.getRentPriceFrom() != null && buildingSearchRequest.getRentPriceFrom() == 0) {
            buildingSearchRequest.setRentPriceFrom(null);
        }

        if (buildingSearchRequest.getRentPriceTo() != null && buildingSearchRequest.getRentPriceTo() == 0) {
            buildingSearchRequest.setRentPriceTo(null);
        }
    ...
    @GetMapping(value = "/chi-tiet-toa-nha-{id}")
    public ModelAndView buildingDetail(@PathVariable("id") Long id) {
        ModelAndView mav = new ModelAndView("web/detail"); // Trỏ đến file JSP mới
